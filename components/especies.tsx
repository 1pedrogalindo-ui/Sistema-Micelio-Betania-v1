...
import {
  ...
  CheckCircle2, ChevronRight, Search, Filter, Sparkles
} from 'lucide-react';
...
export default function Especies() {
  ...
  if (loading) {
    return (
      <div className="space-y-6">
        <div className="text-tierra-600">Cargando catálogo de especies...</div>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="mb-card p-6 bg-red-50 border-red-200">
    ...
    <div className="space-y-6">
      ...
      <header>
        ...
        <p className="text-tierra-600">
          {especies.length} especies disponibles. Cada una con sus fases, parámetros ambientales y receta de sustrato.
        </p>
      </header>
...
function CardEspecie({ especie, onClick }: { especie: Especie; onClick: () => void }) {
  ...
  return (
    <button
      onClick={onClick}
      className="text-left bg-white rounded-2xl border-2 border-micelio-200 p-5 hover:border-micelio-400 hover:shadow-lg transition group"
    >
      {/* Top: badge prioridad */}
      <div className="flex items-start justify-between mb-3">
        <span className={`text-[10px] px-2 py-1 rounded-full font-medium ${badge.color}`}>
          {badge.text}
        </span>
        <span className={`text-xs px-2 py-1 rounded-full border ${dificultadColor}`}>
          Dif. {especie.dificultad}
        </span>
      </div>

      {/* Nombre */}
      <div className="mb-3">
        <h3 className="font-serif text-xl text-tierra-900 leading-tight mb-1">
      ...
      {/* Descripción */}
      {especie.descripcion && (
        <p className="text-sm text-tierra-600 mb-4 line-clamp-2">
          {especie.descripcion}
        </p>
      )}
      ...
      <div className="pt-3 border-t border-micelio-100">
        <div className="flex items-center justify-between">
          ...
          <span className="text-sm font-medium text-micelio-700">
            ${ingresoEstimado.toFixed(0)} USD
          </span>
        </div>
      </div>
...
function ModalDetalleEspecie({ especie, onClose }: { especie: Especie; onClose: () => void }) {
  ...
  return (
    <div
      className="fixed inset-0 bg-tierra-900/60 backdrop-blur-sm z-50 flex items-end md:items-center justify-center p-0 md:p-6"
      onClick={onClose}
    >
      <div
        className="bg-micelio-50 rounded-t-3xl md:rounded-3xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col shadow-2xl"
      ...
      >
        ...
        <div className="bg-gradient-to-r from-tierra-900 to-tierra-800 text-micelio-50 p-6 relative">
          <button
            onClick={onClose}
            className="absolute top-4 right-4 w-8 h-8 rounded-full bg-tierra-700 hover:bg-tierra-600 flex items-center justify-center"
          >
            <X className="w-4 h-4" />
          </button>

          <div className="text-xs uppercase tracking-widest text-micelio-300 mb-2">
            Detalle de Especie
          </div>
        ...
        {/* TABS */}
        <div className="border-b border-micelio-200 px-6 bg-white flex gap-1">
          <TabButton active={tab === 'fases'} onClick={() => setTab('fases')} icon={Clock}>
            Fases y parámetros ({fases.length})
          </TabButton>
          <TabButton active={tab === 'sustrato'} onClick={() => setTab('sustrato')} icon={Beaker}>
            Sustrato ({sustratos.length})
          </TabButton>
          <TabButton active={tab === 'riesgos'} onClick={() => setTab('riesgos')} icon={Shield}>
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {loading ? (
            <div className="text-center text-tierra-500 py-8">Cargando detalles...</div>
          ) : (
            <>
              {tab === 'fases' && (
                <TabFases fases={fases} parametros={parametros} />
              )}
              {tab === 'sustrato' && (
                <TabSustrato sustratos={sustratos} especie={especie} />
              )}
              {tab === 'riesgos' && (
                <TabRiesgos riesgos={riesgos} />
              )}
            </>
          )}
        </div>
        ...
        <div className="p-6 border-t border-micelio-200 bg-white">
          {especie.notas_estrategicas && (
            <div className="mb-4 p-3 bg-micelio-100 rounded-lg">
              <div className="text-xs uppercase tracking-wider text-micelio-700 mb-1">
                Nota estratégica
              </div>
          ...
          <button
            disabled
            className="w-full px-4 py-3 bg-micelio-600 text-white rounded-lg text-sm font-medium hover:bg-micelio-700 disabled:opacity-50 disabled:cursor-not-allowed"
            title="Próximamente"
          >
...
function TabFases({ fases, parametros }: { fases: EspecieFase[]; parametros: EspecieParametro[] }) {
  if (fases.length === 0) {
    return <div className="text-tierra-500 text-sm">No hay fases definidas para esta especie.</div>;
  }
        ...
        return (
          <div key={fase.id} className="bg-white rounded-xl border border-micelio-200 p-4">
            <div className="flex items-start gap-4">
              <div className="w-10 h-10 rounded-full bg-micelio-100 text-micelio-700 flex items-center justify-center font-medium flex-shrink-0">
                {i + 1}
              </div>
              <div className="flex-1">
                <div className="flex items-start justify-between gap-2 mb-1">
                  <div>
                    ...
                    <div className="text-xs text-tierra-500">
                      Duración: {fase.duracion_dias_min}–{fase.duracion_dias_max} días
                    </div>
                  </div>
                ...
                {fase.actividades.length > 0 && (
                  <details className="mb-3">
                    <summary className="text-xs text-micelio-700 cursor-pointer hover:text-micelio-900">
                      Ver {fase.actividades.length} actividades
                    </summary>
                ...
                {paramsFase.length > 0 && (
                  <div className="mt-3 grid grid-cols-2 md:grid-cols-3 gap-2">
                    {paramsFase.map(p => (
                      <ParametroBadge key={p.id} param={p} />
                    ))}
                  </div>
                )}
              </div>
...
function TabSustrato({ sustratos, especie }: { sustratos: EspecieSustrato[]; especie: Especie }) {
  if (sustratos.length === 0) {
    return <div className="text-tierra-500 text-sm">No hay receta de sustrato definida.</div>;
  }
  ...
  return (
    <div className="space-y-4">
      <div className="bg-micelio-100 rounded-xl p-4">
        ...
        <div className="font-serif text-lg text-tierra-900 capitalize">
          {especie.metodo_preparacion}
        </div>
      </div>
...
function TabRiesgos({ riesgos }: { riesgos: EspecieRiesgo[] }) {
  if (riesgos.length === 0) {
    return <div className="text-tierra-500 text-sm">No hay riesgos definidos para esta especie.</div>;
  }
    ...
    <div className="space-y-2">
      {riesgosOrdenados.map(r => (
        <div key={r.id} className="bg-white rounded-xl border border-micelio-200 p-4">
          <div className="flex items-start justify-between gap-3 mb-2">
            ...
            <div className="flex gap-1.5">
              <span className={`text-[10px] px-2 py-0.5 rounded-full ${
                ...
                r.probabilidad === 'media' ? 'bg-amber-100 text-amber-700' :
                'bg-green-100 text-green-700'
              }`}>
                P: {r.probabilidad}
              </span>
              <span className={`text-[10px] px-2 py-0.5 rounded-full ${
                r.impacto === 'critico' ? 'bg-red-200 text-red-800' :
                r.impacto === 'alto' ? 'bg-red-100 text-red-700' :
                r.impacto === 'medio' ? 'bg-amber-100 text-amber-700' :
                'bg-green-100 text-green-700'
              }`}>
                I: {r.impacto}
              </span>
          ...
          {r.sintomas && (
            <div className="text-xs text-tierra-600 mb-2">
              <span className="text-tierra-500">Síntomas:</span> {r.sintomas}
            </div>
          )}

          {r.prevencion && (
            <div className="text-xs text-tierra-700 mb-1">
              <span className="font-medium">🛡️ Prevención:</span> {r.prevencion}
            </div>
          )}

          {r.accion_correctiva && (
            <div className="text-xs text-tierra-700">
              <span className="font-medium">⚡ Acción correctiva:</span> {r.accion_correctiva}
            </div>
          )}
        </div>
      ))}
...
function TabButton({ active, onClick, icon: Icon, children }: any) {
  return (
    <button
      onClick={onClick}
      className={`px-4 py-3 flex items-center gap-2 text-sm border-b-2 transition ${
        active
          ? 'border-micelio-600 text-micelio-700 font-medium'
          : 'border-transparent text-tierra-600 hover:text-tierra-800'
      }`}
    >
...
function UrgenciaBadge({ urgencia }: { urgencia: string }) {
  ...
  return (
    <span className={`text-[10px] px-2 py-0.5 rounded-full ${colores[urgencia] || colores.media}`}>
      {urgencia}
    </span>
  );
}
...
function ParametroBadge({ param }: { param: EspecieParametro }) {
  const iconos: Record<string, any> = {
    temperatura_aire: Thermometer,
    temperatura_sustrato: Thermometer,
  ...
  const labels: Record<string, string> = {
    temperatura_aire: 'T° aire',
    temperatura_sustrato: 'T° sustrato',
    humedad_relativa: 'HR',
    co2: 'CO₂',
    luz_horas: 'Luz',
    ventilacion_renovaciones_hora: 'Vent.',
    ph: 'pH',
  };
  ...
  return (
    <div className={`flex items-center gap-2 px-2 py-1.5 rounded-lg ${
      isCritico ? 'bg-red-50 border border-red-200' : 'bg-micelio-50'
    }`}>
      ...
      <div className="text-xs">
        ...
        <div className="font-medium text-tierra-900">
          {param.valor_min}–{param.valor_max} {param.unidad}
        </div>
      </div>
    </div>
  );
